...
exports.pdf = async (req, res) => {
  try {
    ...
    // const logo = require('./shoplogo.png');
    ...
    // const contentAttributes = await req.body.contentAttributes;
    // const content=``
    // pdfDoc.text(content);
    ...
    function generateCustomerInformation(doc, invoice) {
      ...
      doc
        ...
        .text(formatCurrency(invoice.finalTotal), 150, customerInformationTop + 30)

        .font('Helvetica-Bold')
        .text(invoice.name, 300, customerInformationTop)
        .font('Helvetica')
        .text(invoice.address, 300, customerInformationTop + 15)
        .text(invoice.phone, 300, customerInformationTop + 30)
        .moveDown();
    ...
    function generateInvoiceTable(doc, invoice) {
      ...
      for (i = 0; i < invoice.product.length; i++) {
        ...
        generateTableRow(doc, position, item.products.productcode, item.products.productname, item.products.price, item.amount, formatCurrency(item.products.price * item.amount));
      ...
      doc.font('Helvetica-Bold');
      generateTableRow(doc, duePosition, '', '', 'Balance Due', '', formatCurrency(invoice.finalTotal));
      doc.font('Helvetica');
    }
    ...
    res.status(400).json({ message: 'An error occured in process--' + err.message });
  }
};
